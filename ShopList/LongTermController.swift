...
class LongTermController: SLKTextViewController {
    ...
    override func didPressRightButton(sender: AnyObject!) {
        let shopItem = ShopItem(value: [self.textView.text, "long", NSDate()])
    ...
    func reloadTable() {
        do {
            let realm = try! Realm()
            itemSource = realm.objects(ShopItem).filter("itemType = 'long'").sorted("timeOfInsert", ascending: false)
    ...
    override func tableView(tableView: UITableView, editActionsForRowAtIndexPath indexPath: NSIndexPath) -> [UITableViewRowAction]? {
        let delete = UITableViewRowAction(style: .Normal, title: "Delete") { action, index in
            ...
            deleteFromRealm(cellText!, itemType: "long") //todo: if no error: remove from rows
